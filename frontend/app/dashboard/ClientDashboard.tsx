...
// Shape aligned with `ContestCalendar` and `ContestList` props
type Contest = {
  _id: string;
  id: string;
  platform: string;
  name: string;
  startTime: string; // components expect string dates
  endTime: string;
  url: string;
};
  ...
  const loadContests = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- normalizing unknown backend shape into local Contest type
        const rawList: any[] = Array.isArray(data.contests) ? data.contests : [];
        const seen = new Set<string>();
        const normalized: Contest[] = [];
        for (const c of rawList) {
          const id = String(c.id ?? `${c.platform}:${c.name}:${c.startTime}`);
          ...
          normalized.push({
            ...
            endTime: String(c.endTime ?? c.startTime ?? ""),
            url: String(c.url ?? "#"),
          });
        }
