...
export default function CalendarPreview({ contests }: CalendarPreviewProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-200 dark:border-slate-700 shadow"
    >
      ...
      {upcomingContests.length === 0 ? (
        <div className="text-center py-8">
          <div className="w-12 h-12 bg-gray-100 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-3">
            <Calendar className="w-6 h-6 text-gray-500 dark:text-gray-300" />
          </div>
        ...
        <div className="space-y-3">
          {upcomingContests.map((contest, index) => (
            <motion.div
              ...
              className="bg-gray-50 dark:bg-slate-700 rounded-xl p-4 border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 transition-all duration-200"
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <span className={`inline-block w-2 h-2 rounded-full ${getPlatformColor(contest.platform)}`}></span>
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{contest.platform}</span>
                </div>
                <a
                  href={contest.url}
                  target="_blank"
